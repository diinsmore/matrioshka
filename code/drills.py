...
class Drill(MachineSpriteBase):
    def __init__(
        ...
        self.alarms = {
            'extract': Alarm(2000 * self.speed_factor * self.extract_time_factor * (self.ore_row + 1), self.extract, loop=True, track_pct=True),
            'burn fuel': Alarm(2000 * self.speed_factor * self.extract_time_factor * (self.ore_row + 1), self.extract, loop=True, track_pct=True),
        }
